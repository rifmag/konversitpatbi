...
# Fungsi untuk membuat barcode signature dan menyimpannya sebagai gambar
def create_barcode_signature():
    qr = qrcode.QRCode(
        version=1,
    ...
    qr.add_data("Contoh Tanda Tangan")  # Ganti dengan data tanda tangan Anda
    qr.make(fit=True)

    img = qr.make_image(fill_color="black", back_color="white")
    img.save("signature_qr.png")  # Simpan gambar barcode signature

# Fungsi untuk membuat PDF dengan gambar barcode signature
def create_pdf_with_barcode_signature():
    pdf = SimpleDocTemplate("hasil_tanda_tangan.pdf", pagesize=letter)
    styles = getSampleStyleSheet()

    story = []

    create_barcode_signature()  # Membuat barcode signature

    # Tambahkan gambar barcode signature ke PDF
    img = Image("signature_qr.png", width=100, height=100)
    story.append(img)

    doc = SimpleDocTemplate("hasil_tanda_tangan.pdf", pagesize=letter)
    doc.build(story)
...
# Simpan hasil dalam PDF
    ...
    # Tambahkan tabel
        ...
        # Tambahkan QR code di bawah "Best Regards"
        qr_img = "signature_qr.png"  # Ganti dengan path gambar QR code Anda
        pdf.image(qr_img, x=10, y=pdf.get_y() + 10, w=25)  # Sesuaikan posisi dan ukuran QR code
...
def add_bg_from_url():
    st.markdown(
         ...
         <style>
         .stApp {{
             background-image: url("https://cdn.pixabay.com/photo/2021/08/10/18/16/green-6536646_640.png");
